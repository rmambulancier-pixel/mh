/* MesHeures V25 — sauvegarde locale renforcée, JSON versionné et restauration sûre */
(function(){
  const BACKUP_VERSION='25.0.0';
  const PREFIX=LS+'_v25_backup_';
  const PREV_PREFIX=LS+'_v24_backup_';
  ...
  function listKeys(){
    return Object.keys(localStorage)
      .filter(k=>k.indexOf(PREFIX)===0 || k.indexOf(PREV_PREFIX)===0 || k.indexOf(LEGACY_V23_PREFIX)===0 || k.indexOf(LEGACY_PREFIX)===0 || k.indexOf(LEGACY_V17_PREFIX)===0)
      .sort()
      .reverse();
  }
  function prune(){
  ...
  function stamp(){
    ...
    localStorage.setItem(LS+'_v25_last',now);
  ...
  window.mhV17BackupStatus=function(){
    const keys=listKeys();
    let last=localStorage.getItem(LS+'_v25_last')||localStorage.getItem(LS+'_v23_last')||localStorage.getItem(LS+'_v21_last')||localStorage.getItem(LS+'_v18_last')||localStorage.getItem(LS+'_v17_last')||'';
    if(!last&&keys.length){
  ...
  window.mhV17Export=function(){
    try{
      ...
      const mode=window.mhDownloadFile('MesHeures-backup-V25-'+new Date().toISOString().slice(0,10)+'.json',JSON.stringify(s,null,2),'application/json;charset=utf-8');
      return !!mode;
  ...
  window.mhV17ListBackups=function(){
    return listKeys().map(k=>{
      try{const s=JSON.parse(localStorage.getItem(k));return {key:k,date:s.createdAt,version:s.version||'25.0.0',reason:s.reason||''};}
      catch(e){return null;}
    }).filter(Boolean);
  };
