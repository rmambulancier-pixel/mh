/* MesHeures Paie — Paie / Quatorzaines
 ...
 * - résumé copiable
 * - version d’interface synchronisée avec le socle actuel
 *
...
(function () {
  ...
  function syncVersion() {
    document.title = document.title.replace(/V16\.1(?:\.0)?|V16\.2(?:\.0)?|V17(?:\.0\.1)?/g, 'V24.2.0');
    ...
    nodes.forEach(n => {
      if (/V16\.1\.0|V16\.2\.0/.test(n.nodeValue || '')) {
        n.nodeValue = n.nodeValue
          .replace(/V16\.1\.0/g,'V24.2.0')
          .replace(/V16\.2\.0/g,'V24.2.0');
      }
    });
  }
  ...
  function setQuatorzaines(value) {
    ...
    updatePeriodLabel();
    if (typeof mhRefresh === 'function') mhRefresh('pay-period-change');
...
/* V24.2.0 — le résumé de paie canonique est exposé par app-core.js.
