...
(function () {
  ...
  function syncVersion() {
    document.title = document.title.replace(/V16\.1(?:\.0)?|V16\.2(?:\.0)?|V17(?:\.0\.1)?/g, 'V21.0.0');
    const meta = document.querySelector('meta[name="application-version"]');
    if (meta) meta.setAttribute('content','21.0.0');
    ...
    nodes.forEach(n => {
      if (/V16\.1\.0|V16\.2\.0/.test(n.nodeValue || '')) {
        n.nodeValue = n.nodeValue
          .replace(/V16\.1\.0/g,'V21.0.0')
          .replace(/V16\.2\.0/g,'V21.0.0');
      }
    });
  }
...
/* V21.0.0 — le résumé de paie canonique est exposé par app-core.js.
