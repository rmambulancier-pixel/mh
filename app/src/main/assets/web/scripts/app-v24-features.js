...
// Démarrage automatique : dès que l'heure de Début saisie pour aujourd'hui est atteinte
// (ou déjà dépassée), le service passe seul en "EN SERVICE" sans clic sur Démarrer.
function checkAutoStart(){
  const k=today();const d=DB.days?.[k];
  if(!d||d.running||d.fin)return;
  if((d.t!=='T'&&d.t!=='NUIT')||!d.deb)return;
  const nowM=P(nowHHMM()),debM=P(d.deb);
  if(nowM==null||debM==null||nowM<debM)return;
  const sd=new Date();sd.setHours(Math.floor(debM/60),debM%60,0,0);
  d.running=true;d.startEpoch=sd.getTime();d.endEpoch=null;
  save();notify();
}
