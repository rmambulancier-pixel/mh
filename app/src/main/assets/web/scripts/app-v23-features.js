/* MesHeures V23 — operational extensions: proactive alerts, live timer, structured OCR, CPH and TPU. */
(function(){
'use strict';
...
const V='23.0.0', WEEK48=2880, AVG46=2760, REST11=660;
...
function renderProactive(){
  ['mhV23Proactive','mhV23HomeProactive'].forEach(id=>{const h=document.getElementById(id);if(!h)return;const a=scan();h.innerHTML=a.length?a.slice(0,5).map(x=>`<div class="mh-v23-issue ${x.lvl}"><b>${x.lvl==='bad'?'🔴':'🟠'} ${esc0(x.title)}</b><small>${esc0(x.text)}</small><em>${esc0(x.rule)}</em></div>`).join(''):'<div class="mh-v23-ok">🟢 Aucun dépassement prévisible dans les données actuellement saisies.</div>'});
}
let lastToast='';function notify(){const a=scan();if(!a.length)return;const key=a.map(x=>x.id).join('|');if(key===lastToast)return;lastToast=key;let t=document.getElementById('mhV23Toast');if(!t){t=document.createElement('div');t.id='mhV23Toast';document.body.appendChild(t)}t.className='mh-v23-toast '+(a.some(x=>x.lvl==='bad')?'bad':'warn');t.textContent=a[0].title+' — '+a[0].text;t.classList.add('on');clearTimeout(notify.t);notify.t=setTimeout(()=>t.classList.remove('on'),4500)}
...
function renderTimer(){const h=document.getElementById('mhV23Timer');if(!h)return;const d=DB.days?.[curDate];if(!d||d.t!=='T'){h.innerHTML='';return}if(d.running){h.innerHTML=`<div class="mh-v23-live"><span class="pulse"></span><div><b>EN SERVICE</b><strong id="mhV23Elapsed">${fmt(elapsed(curDate))}</strong><small>Début ${esc0(d.deb||'—')} · comptage automatique</small></div><button class="danger-soft" onclick="mhV23StopTimer()">⏹ Arrêter</button></div>`}else if(d.deb&&!d.fin){h.innerHTML=`<div class="mh-v23-live pending"><div><b>Journée ouverte</b><strong>${fmt(elapsed(curDate))}</strong><small>Début ${esc0(d.deb)}</small></div><button onclick="mhV23StartTimer()">▶ Reprendre</button></div>`}else h.innerHTML='<button class="mh-v23-start" onclick="mhV23StartTimer()">▶ Démarrer le chronomètre</button>'}
...
function renderTPU(){const x=tpu();document.querySelectorAll('[data-mh-v23-tpu]').forEach(h=>h.innerHTML=`<div class="mh-v23-tpu-main"><strong>${x.p.toFixed(1)} %</strong><span>TTE / temps mobilisé</span></div><div class="mut">${fmt(x.u)} utiles · ${fmt(x.m)} mobilisées · ${x.n} journées.</div><small class="mut">Indicateur personnel expérimental, pas une mesure réglementaire.</small>`)}
...
function inject(){
  if(!document.getElementById('mhV23ExtraBlock')){
    const host=document.createElement('div');
    host.id='mhV23ExtraBlock';
    host.innerHTML=`<div class="card"><div class="section-head"><h2>🛡️ Alerte proactive</h2><span class="period-pill">temps réel</span></div><div id="mhV23Proactive"></div></div><div class="card"><div class="section-head"><h2>📐 Taux de Présence Utile</h2><span class="period-pill">personnel</span></div><div data-mh-v23-tpu></div></div><div class="card"><div class="section-head"><h2>⚖️ Pièce CPH</h2><span class="period-pill">1 clic</span></div><p class="mut">Synthèse par quatorzaine avec renvois aux articles du référentiel MesHeures et empreinte SHA-256.</p><button onclick="mhV23ExportCPH()" style="width:100%">📄 Générer la pièce CPH</button></div>`;
  ...
  if(!document.getElementById('mhV23Timer')){
    const h=document.createElement('div');h.id='mhV23Timer';
    const j=document.getElementById('s-jour');if(j)j.insertBefore(h,j.querySelector('.card'));
  }
  if(!document.getElementById('mhV23BulFile')){
    const bul=document.getElementById('s-bul'),c=bul?.querySelector('.card');
    if(c){
      const b=document.createElement('div');b.className='card';
      b.innerHTML=`<div class="section-head"><h2>🔎 OCR bulletin scanné</h2><span class="period-pill">structuré</span></div><p class="mut">Pour les bulletins image/scannés : extraction OCR puis rapprochement des lignes sensibles.</p><button class="g" style="width:100%" onclick="document.getElementById('mhV23BulFile').click()">📎 Scanner le PDF</button><input id="mhV23BulFile" type="file" accept=".pdf,application/pdf,image/*" style="display:none" onchange="mhV23OCRBulletin(this.files[0])"><div id="mhV23BulOCR" style="margin-top:9px"></div>`;
      c.after(b);
    }
  }
  if(!document.getElementById('mhV23HomeProactive')){
    const home=document.getElementById('s-home');
    if(home){
      const c=document.createElement('div');c.className='card';c.id='mhV23Home';
      c.innerHTML=`<div class="section-head"><h2>🛡️ Anticipation</h2><button class="text-action" onclick="tab('analyse')">Voir ›</button></div><div id="mhV23HomeProactive"></div>`;
      home.appendChild(c);
    }
  }
}
function refresh(){renderProactive();renderTPU();renderTimer()}
function patch(name,after){const key='__mhV23_'+name;if(window[key]||typeof window[name]!=='function')return;const old=window[name];window[name]=function(){const r=old.apply(this,arguments);setTimeout(()=>{after();refresh();},0);return r};window[key]=true}
...
window.mhV23StartTimer=startTimer;window.mhV23StopTimer=stopTimer;window.mhV23ExportCPH=exportCPH;window.mhV23OCRBulletin=ocrBulletin;window.mhV23ProactiveScan=scan;
