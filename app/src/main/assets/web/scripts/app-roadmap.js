...
let body= r.map(x=>{if(!x.work){let detail=x.note||x.tasks||'';return `<tr class="off status-${x.d.t||"REPOS"}"><td colspan="8"><b>${E(x.label)}</b> — <strong>${E(x.status)}</strong>${x.holiday?' · FÉRIÉ':''}<br><strong>${E(x.status)}</strong>${detail?`<br><small>${E(detail)}</small>`:''}</td></tr>`}return `<tr><td><b>${E(x.label)}</b>${x.holiday?' · FÉRIÉ':''}</td><td>${x.d.deb||'—'}</td><td>${(x.d.p||[]).filter(p=>p&&(p.d||p.f)).map(p=>(p.d||'??')+'–'+(p.f||'??')+(p.ty?' · '+p.ty:'')).join('<br>')||'—'}</td><td>${x.d.fin||'—'}</td><td class=num>${x.work?H(x.amp):'—'}</td><td>${E(x.perm)||'—'}</td><td>${E(x.tasks)||(x.note?'Note : '+E(x.note):'—')}</td><td></td></tr>`}).join('');
...
.num{text-align:center;white-space:nowrap}.off td{background:#f7f7f7;color:#555;font-size:8.5pt;padding:5px 7px}
.off strong{font-size:9pt;color:#222}
...
@media print and (orientation:portrait){
 .off td{font-size:7.5pt;padding:4px 5px}
 .off strong{font-size:8pt}
}
