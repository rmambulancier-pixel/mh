...
function rows(s){let o=[];for(let i=0;i<7;i++){let k=add(s,i),d=DB.days[k]||{t:'REPOS',p:[]},r=typeof cd==='function'?cd(k):{tte:0},a=min(d.deb),b=min(d.fin),work=(d.t==='T'||d.t==='NUIT')&&a!=null&&b!=null,amp=work?((b<=a?b+1440:b)-a):0;o.push({k,d,r,work,label:D[i]+" "+SD(k),status:d.t==='CP'?'CONGÉ PAYÉ':d.t==='RC'?'REPOS COMPENSATEUR':d.t==='MAL'?'MALADIE':d.t==='NUIT'?'NUIT':d.t==='T'?'TRAVAIL':'REPOS',amp,tte:work?Math.max(0,+r.tte||0):0,pause:pm(d),max:mc(d),meal:work?meal(d):null,sa:work?a+i*1440:null,ea:work?((b<=a?b+1440:b)+i*1440):null,holiday:!!d.fer,perm:d.perm||d.permanence||'',tasks:d.tasks||d.taches||d.activites||'',note:d.note||''})}return o}
...
let body= r.map(x=>{if(!x.work){let detail=x.note||x.tasks||'';return `<tr class="off"><td><b>${E(x.label)}</b>${x.holiday?' · FÉRIÉ':''}<br><strong>${E(x.status)}</strong>${detail?`<br><small>${E(detail)}</small>`:''}</td><td>—</td><td>—</td><td>—</td><td class=num>—</td><td>—</td><td>${E(detail)||'—'}</td><td></td></tr>`}return `<tr><td><b>${E(x.label)}</b>${x.holiday?' · FÉRIÉ':''}</td><td>${x.d.deb||'—'}</td><td>${(x.d.p||[]).filter(p=>p&&(p.d||p.f)).map(p=>(p.d||'??')+'–'+(p.f||'??')+(p.ty?' · '+p.ty:'')).join('<br>')||'—'}</td><td>${x.d.fin||'—'}</td><td class=num>${x.work?H(x.amp):'—'}</td><td>${E(x.perm)||'—'}</td><td>${E(x.tasks)||(x.note?'Note : '+E(x.note):'—')}</td><td></td></tr>`}).join('');
...
.num{text-align:center;white-space:nowrap}.off td{background:#f7f7f7;color:#555}.off strong{font-size:7.5pt}
...
Document généré par MesHeures et structuré selon le modèle réglementaire. Références : arrêté du 19 décembre 2001 modifié le 18 août 2009 · Code des transports R3312-30, D3312-31, D3312-32, R3312-33 · accord du 16 juin 2016. Contrôles = aide à la vérification, à partir des données saisies.</div>
</body></html>`}
