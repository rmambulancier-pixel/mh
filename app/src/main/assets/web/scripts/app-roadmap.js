...
function rows(s){let o=[];for(let i=0;i<7;i++){let k=add(s,i),d=DB.days[k]||{t:'REPOS',p:[]},r=typeof cd==='function'?cd(k):{tte:0},a=min(d.deb),b=min(d.fin),work=(d.t==='T'||d.t==='NUIT')&&a!=null&&b!=null,amp=work?((b<=a?b+1440:b)-a):0;o.push({k,d,r,work,label:D[i]+" "+SD(k),amp,tte:work?Math.max(0,+r.tte||0):0,pause:pm(d),max:mc(d),meal:work?meal(d):null,sa:work?a+i*1440:null,ea:work?((b<=a?b+1440:b)+i*1440):null,holiday:!!d.fer,perm:d.perm||d.permanence||'',tasks:d.tasks||d.taches||d.activites||'',note:d.note||''})}return o}
...
function make(s){let r=rows(s),a=checks(r),amp=r.reduce((x,y)=>x+y.amp,0),tte=r.reduce((x,y)=>x+y.tte,0),body=r.map(x=>`<tr><td><b>${E(x.label)}</b>${x.holiday?' · FÉRIÉ':''}</td><td>${x.d.deb||'—'}</td><td>${(x.d.p||[]).filter(p=>p&&(p.d||p.f)).map(p=>(p.d||'??')+'–'+(p.f||'??')+(p.ty?' · '+p.ty:'')).join('<br>')||'—'}</td><td>${x.d.fin||'—'}</td><td class=num>${x.work?H(x.amp):'—'}</td><td>${E(x.perm)||'—'}</td><td>${E(x.tasks)||(x.note?'Note : '+E(x.note):'—')}</td><td></td></tr>`).join(''),alerts=a.length?a.map(x=>`<div class="a ${x[0]}"><b>${x[0]==='critical'?'ANOMALIE':'À VÉRIFIER'}</b> · ${E(x[1])}</div>`).join(''):'<div class="a ok"><b>OK</b> · Aucun contrôle de base en anomalie.</div>';
