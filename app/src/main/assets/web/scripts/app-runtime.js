/* MesHeures V20.5.0 — interface, projection, conformité et migrations.
...
(function(){
  ...
  function theme(t){
    ...
    if(window.MesHeuresAndroid&&typeof window.MesHeuresAndroid.setSystemBarsLight==='function')window.MesHeuresAndroid.setSystemBarsLight(resolved==='light');
  ...
  window.mhV17Legal=function(){
    ...
    const box=document.createElement('div');box.className='mh-v17-overlay';box.innerHTML=`<div class="mh-v17-dialog"><div class="mh-v17-sheet-head"><b>⚖️ Contrôle légal</b><button class="g" onclick="this.closest('.mh-v17-overlay').remove()">✕</button></div><p class="mut">Référentiel vérifié le 11/09/2026 · ${MH_LEGAL.ccn}</p><div class="mh-v17-legal-list">${al.length?al.map(a=>`<div class="al ${a.lvl}"><b>${a.k}</b> — ${esc(a.m)}<small>${esc(a.rule||'')}</small></div>`).join(''):'<div class="al k">✅ Aucun écart détecté par les contrôles sur la période.</div>'}</div><hr><div class="mut">Minima ambulanciers : N1 ${MH_LEGAL.rules.ccnMin.n1.toFixed(2)} € · N2 ${MH_LEGAL.rules.ccnMin.n2.toFixed(2)} € · N3 ${MH_LEGAL.rules.ccnMin.n3.toFixed(2)} €. SMIC : ${MH_LEGAL.rules.smic.value.toFixed(2)} €.</div></div></div>`;document.body.appendChild(box);
  };
  window.mhV17BackupPanel=function(){
    ...
    box.innerHTML=`<div class="mh-v17-dialog">
      <div class="mh-v17-sheet-head"><b>💾 Sauvegarde renforcée V20.5.0</b><button class="g" onclick="this.closest('.mh-v17-overlay').remove()">✕</button></div>
      ...
      <div class="row" style="margin-top:7px">
        <button class="g" onclick="if(typeof mhV18EncryptedBackup==='function')mhV18EncryptedBackup();else alert('Le backup chiffré est indisponible sur cet écran.')">🔐 Export chiffré</button>
      </div>
      <div class="al i">Avant chaque import/restauration, MesHeures crée automatiquement un point de sécurité. Jusqu’à 5 points locaux sont conservés. Les anciens points V17 restent restaurables.</div>
  ...
  function boot(){migrate();theme(DB.s.theme);if(document.getElementById('mhVersion'))document.getElementById('mhVersion').textContent='V20.5.0';buildNav();patchHome();patchRender();addReg();legalCard();injectHome();setTimeout(()=>{try{tab('home');if(typeof pushWidgetData==='function')pushWidgetData()}catch(e){console.error(e)}},0);}
