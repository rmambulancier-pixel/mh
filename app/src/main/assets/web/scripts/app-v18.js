...
(function(){
  ...
  window.mhV17BackupPanel=function(){
    ...
    box.innerHTML=`<div class="mh-v17-dialog">
      <div class="mh-v17-sheet-head"><b>💾 Sauvegarde renforcée V18.0.20</b><button class="g" onclick="this.closest('.mh-v17-overlay').remove()">✕</button></div>
  ...
  function boot(){migrate();theme(DB.s.theme);if(document.getElementById('mhVersion'))document.getElementById('mhVersion').textContent='V18.0.20';buildNav();patchHome();patchRender();addReg();legalCard();injectHome();setTimeout(()=>{try{renderAll();if(typeof pushWidgetData==='function')pushWidgetData()}catch(e){console.error(e)}},0);}
