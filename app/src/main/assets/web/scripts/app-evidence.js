/* MesHeures V30.2.3 — constats, journal de preuves, chronologie, backup chiffré
...
(function(){
  ...
  function renderEvidence(){
    const c=buildCache();
    const host=document.getElementById('mhV30Evidence');if(!host)return;
    host.innerHTML=`<div class="card mh-v30-card"><h2>🛡️ Dossier & preuves</h2><div class="mh-v30-kpis"><div><b>${c.constats.length}</b><span>constat(s)</span></div><div><b>${EUR(c.total)}</b><span>estimation chiffrée</span></div><div><b>${c.events.length}</b><span>événement(s)</span></div></div><div class="row"><button onclick="mhV30ConstatsPDF()">📄 Export constats</button><button class="g" onclick="mhV30AddEvent()">＋ Événement</button><button class="g" onclick="mhV30EncryptedBackup()">🔐 Backup chiffré</button><button class="g" onclick="document.getElementById('mhV30Restore').click()">🔓 Restaurer chiffré</button><input id="mhV30Restore" type="file" accept=".json,.mhbackup" style="display:none" onchange="mhV30EncryptedRestore(this)"></div><details open><summary>📚 Journal de preuves cumulatif</summary><div class="mh-v30-table">${c.rows.length?c.rows.map(x=>`<div class="mh-v30-row"><b>${x.mois}</b><span>Calcul ${F(x.calcMin)}</span><span>Payé ${F(x.paidMin||0)}</span><strong class="${(x.ecartMin||0)>0?'bad':'ok'}">Écart ${F(x.ecartMin||0)}</strong><span>${x.bulletin?`💶 ${EUR(x.ecartEur)}`:'—'}</span></div>`).join(''):'<div class="mut">Aucun bulletin exploitable sur l’historique.</div>'}</div></details><details><summary>🛡️ Constats persistants</summary><div>${c.constats.length?c.constats.map(x=>`<div class="mh-v30-proof"><b>${esc(x.id)}</b> · ${esc(x.jour_concerne)} · ${esc(x.regle_violie)}<small>Créé le ${new Date(x.date_constat).toLocaleString('fr-FR')} · ${esc(x.article_source)}${x.calcul_montant_du?` · ${EUR(x.calcul_montant_du)} estimés`:''}</small></div>`).join(''):'<div class="mut">Aucun constat.</div>'}</div></details><details><summary>🕰️ Frise chronologique</summary><div class="mh-v30-timeline">${[...c.constats.map(x=>({date:x.jour_concerne,type:'Constat',text:x.regle_violie})),...(DB.bulletins||[]).map(b=>({date:b.mois+'-01',type:'Bulletin',text:'Bulletin importé'})),...c.events.map(e=>({date:e.date,type:e.type,text:e.text}))].sort((a,b)=>b.date.localeCompare(a.date)).map(e=>`<div><time>${esc(e.date)}</time><b>${esc(e.type)}</b><span>${esc(e.text)}</span></div>`).join('')||'<div class="mut">Aucun événement.</div>'}</div></details></div>`;
  }

  window.mhV30AddEvent=function(){const date=prompt('Date (AAAA-MM-JJ) :',today());if(!date)return;const type=prompt('Type :','Courrier');if(!type)return;const text=prompt('Description :','');if(!text)return;if(addEvent(date,type,text)){renderEvidence();alert('✅ Événement ajouté.')}};
  window.mhV30ConstatsPDF=function(){
    const c=buildCache();
  ...
  window.mhV30RenderEvidence=renderEvidence;

  function wrapSave(){
    if(window.__mhV30EvidenceSavePatch||typeof window.save!=='function')return;
    window.__mhV30EvidenceSavePatch=true;
    const oldSave=window.save;
    window.save=function(){const result=oldSave.apply(this,arguments);invalidate();return result};
  }
  function inject(){init();wrapSave();if(document.getElementById('mhV30Evidence'))return;const host=document.createElement('section');host.id='mhV30Evidence';const target=document.getElementById('s-audit')||document.body;target.appendChild(host);renderEvidence();}
  const oldRenderAll=window.renderAll;
  if(oldRenderAll&&!window.__mhV30EvidencePatch){window.__mhV30EvidencePatch=true;window.renderAll=function(){oldRenderAll();inject();renderEvidence()}}
