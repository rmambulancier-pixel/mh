...
(function(){
  ...
  window.mh25SetType=function(v){const d=ensureDay(curDate||day());d.t=v;d.p=d.p||[];if(v!=='T'&&v!=='NUIT'){d.running=false;d.startEpoch=0;}if(typeof save==='function')save();renderDay();};
  window.mh25SetPanier=function(v){const d=ensureDay(curDate||day());d.panier=v||null;save();renderDay();};
  window.mh25AddPause=function(){ensureDay(curDate||day()).p.push({d:'',f:'',ty:'ENT'});save();renderDay();};
  window.mh25DelPause=function(i){ensureDay(curDate||day()).p.splice(i,1);save();renderDay();};
  window.mh25SetPause=function(i,f,v){const d=ensureDay(curDate||day());d.p[i]=d.p[i]||{ty:'ENT'};d.p[i][f]=v;save();renderDay();};
  window.mh25SetNote=function(v){ensureDay(curDate||day()).note=v;save();};
  window.mh25SetField=function(f,v){ensureDay(curDate||day())[f]=v;save();renderDay();};
  ...
  function boot(){
    buildAnalysis();buildNav25();patchTab();
    if(el('mhVersion'))el('mhVersion').textContent='V25.0.0';
    document.title='MesHeures V25.0';
    Live.render();
    renderHome25();renderDay25();renderAnalysis();
    if(Live.tick)clearInterval(Live.tick);
    Live.tick=setInterval(()=>{if(Live.active()){Live.render();if(curTab==='home')renderHomeData();if(curTab==='jour')renderDay25();if(curTab==='analyse')renderAnalysis();try{pushWidgetData()}catch(e){}}},1000);
