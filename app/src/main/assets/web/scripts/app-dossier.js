/* MesHeures V24.2.0 — Lot 3 : dossier complet, empreinte d'intégrité et export probatoire */
(function(){
  'use strict';
  ...
  function render(){
    ...
    const sec=document.createElement('section');sec.id='mhV18Dossier';sec.innerHTML=`<div class="card mh-v20-card"><h2>📁 Dossier complet V24.2.0</h2><p class="mut">Regroupe l'historique, les constats, les événements, les bulletins et l'intelligence locale dans un export unique.</p><div class="row"><button onclick="mhV18ExportDossier()">⬇️ Export dossier JSON</button><button class="g" onclick="mhV18PrintDossier()">🖨️ Dossier imprimable / PDF</button><button class="g" onclick="mhV30ExportSignedPDF()">🔏 PDF probatoire signé</button></div><div id="mhV18DossierHash" class="mut" style="margin-top:8px"></div></div>`;target.appendChild(sec);refreshHash();
  }
