...
check('Release target', version == '26.0.0' and code == '2600')
...
check('HTML title/meta', ('MesHeures V26.0' in index or f'MesHeures V{version}' in index) and f'application-version" content="{version}"' in index)
check('V26 Flow script referenced', ('scripts/app-v26.js' in index or './scripts/app-v26.js' in index) and (SCRIPTS/'app-v26.js').exists())
check('V26 stylesheet referenced', './style/v26.css' in index and (WEB/'style/v26.css').exists())
...
check('Backup namespace current', "BACKUP_VERSION='26.0.0'" in backup and "LS+'_v26_backup_'" in backup)
check('Backup previous compatibility', "LS+'_v24_backup_'" in backup and "LS+'_v23_backup_'" in backup)
check('V26 unified orchestrator', (SCRIPTS/'app-v26.js').exists() and 'window.MH26' in text(SCRIPTS/'app-v26.js') and 'window.tab=function' in text(SCRIPTS/'app-v26.js'))
check('V26 single live scheduler', 'setInterval' in text(SCRIPTS/'app-v26.js') and 'clearInterval' in text(SCRIPTS/'app-v26.js'))
check('V25 smart time compatibility', 'normalizeTimeInput' in app25 and 'data-time-smart' in app25)
check('V26 compact navigation', all(x in text(SCRIPTS/'app-v26.js') for x in ['Accueil','Saisie','Planning','Paie','Analyse']))
...
check('V26 Application bootstrap', 'MesHeuresApplication' in main or (ROOT / 'app/src/main/java/com/mesheures/app/MesHeuresApplication.java').exists())
