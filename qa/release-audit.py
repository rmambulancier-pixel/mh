...
check('Release target', version == '20.5.0' and code == '2050')
...
check('File access hardened', 'setAllowFileAccess(false)' in hybrid and 'setAllowContentAccess(false)' in hybrid)
check('Lifecycle backup guard', 'if (web == null || !webReady) return;' in main)
check('No legacy 30s full-storage timer', 'setInterval(save, 30000)' not in main and 'setInterval(save,30000)' not in main)
check('Backup namespace current', "LS+'_v20_5_backup_'" in backup and "BACKUP_VERSION='20.5.0'" in backup)
...
for phrase in [
    ...
    'Backup V18', 'sauvegarde renforcée V20.4.0',
]:
