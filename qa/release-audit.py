...
check('Release target',version=='28.0.0' and code=='2800')
...
check('HTML title/meta','MesHeures V28.0' in index and f'application-version" content="{version}"' in index)
check('V28 runtime referenced','scripts/app-v28.js' in index and (S/'app-v28.js').exists())
check('V28 stylesheet referenced','./style/v28.css' in index and (WEB/'style/v28.css').exists())
...
check('Backup namespace',"BACKUP_VERSION='28.0.0'" in backup and "LS+'_v28_backup_'" in backup and "LS+'_v27_backup_'" in backup)
check('Backup compatibility','LS+\'_v26_backup_\'' in backup and 'LEGACY_V25_PREFIX' in backup and 'LEGACY_V24_PREFIX' in backup)
check('V28 reactive orchestrator','window.MH28' in v28 and 'requestAnimationFrame' in v28 and 'setTimeout(liveTick,1000)' in v28 and 'window.tab=navigation' in v28 and 'MH28DataEngine' in v28)
